...
controller_interface::CallbackReturn ElastoplasticController::on_configure(const rclcpp_lifecycle::State& /*previous_state*/) {
  ...
  if (m_parameters.debug.pub) {
    ...
    m_pub_fk_world_tool[Side::LEFT] =
      this->get_node()->create_publisher<geometry_msgs::msg::PoseStamped>("~/fk_world_tool/left", rclcpp::QoS(10));
    m_pub_fk_world_tool[Side::RIGHT] =
      this->get_node()->create_publisher<geometry_msgs::msg::PoseStamped>("~/fk_world_tool/right", rclcpp::QoS(10));
...
controller_interface::CallbackReturn ElastoplasticController::on_activate(const rclcpp_lifecycle::State& /*previous_state*/) {
  ...
  if (m_parameters.debug.pub) {
    ...
    m_pub_fk_world_tool[Side::LEFT]->on_activate();
    m_pub_fk_world_tool[Side::RIGHT]->on_activate();
...
#define USE_CARTESIAN_REFERENCE
  ...
  if (m_parameters.debug.pub) {
    ...
    qepp_msg.data.resize(m_qpp.size());
    std::ranges::copy(m_qpp, qepp_msg.data.begin());
    ...
    msg_wrench_shared.header.stamp = time_now;
    msg_wrench_shared.header.frame_id = m_parameters.frames.map;
    ...
    Eigen::Affine3d fk;
    fk = T_world_tool[Side::LEFT];
    fk_msg.header.stamp = time_now;
    fk_msg.header.frame_id = m_parameters.frames.map;
    fk_msg.pose = Eigen::toMsg(fk);
    m_pub_fk_world_tool[Side::LEFT]->publish(fk_msg);

    fk = T_world_tool[Side::RIGHT];
    fk_msg.header.stamp = time_now;
    fk_msg.header.frame_id = m_parameters.frames.map;
    fk_msg.pose = Eigen::toMsg(fk);
    m_pub_fk_world_tool[Side::RIGHT]->publish(fk_msg);


    fk = m_chain_base_tools[Side::LEFT]->getTransformation(m_q(m_sel[Side::LEFT]).tail(m_nax_s[Side::LEFT]));
    fk_msg.header.stamp = time_now;
    ...
    fk = m_chain_base_tools[Side::RIGHT]->getTransformation(m_q(m_sel[Side::RIGHT]).tail(m_nax_s[Side::RIGHT]));
    fk_msg.pose = Eigen::toMsg(fk);
