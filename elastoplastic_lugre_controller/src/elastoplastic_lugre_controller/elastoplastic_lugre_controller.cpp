...
Eigen::Affine3d
integratePoseLocalWorldAligned(const pinocchio::SE3 &pose,
  ...
  const Eigen::Vector3d p_next = pose.translation() + v_world * dt;
...
void ElastoplasticController::build_pinocchio_model(
  ...
  pinocchio::urdf::buildModel(urdf_model, pinocchio::JointModelPlanar(),
  ...
  unsigned int root_jnts_to_lock =
      attach_mobile_base
          ? 2
          : 1; // In theory, I could simply avoid adding the planar joint
  ...
  std::vector<pinocchio::JointIndex> joint_id_to_lock(
      joint_names_to_lock.size());
  ...
  m_model =
      pinocchio::buildReducedModel(chain_world_tool_full, joint_id_to_lock,
                                   pinocchio::neutral(chain_world_tool_full));
  ...
  std::for_each(m_model.names.begin(), m_model.names.end(),
                [this](const std::string &s) {
...
Eigen::Vector6d
ElastoplasticController::get_wrench_in_world(pinocchio::Data &data) {
  ...
  // RCLCPP_INFO_STREAM(get_node()->get_logger(), "wrench_sensor_in_sensor -> "
  ...
  // RCLCPP_INFO_STREAM(get_node()->get_logger(), "wrench_sensor_in_sensor
  // (after filter) -> " << wrench_sensor_in_sensor.transpose());
  ...
  // RCLCPP_INFO_STREAM(get_node()->get_logger(), "wrench_sensor_in_sensor
  // (after filter and deadband) -> " << wrench_sensor_in_sensor.transpose());
  ...
  const pinocchio::SE3 world_R_tool(world_M_tool.rotation(),
                                    Eigen::Vector3d::Zero());

  // RCLCPP_INFO_STREAM(get_node()->get_logger(), "wrench_tool_in_tool -> " <<
  // wrench_tool_in_tool.toVector());

  Eigen::Vector6d wrench_tool_in_world =
      world_R_tool.act(wrench_tool_in_tool).toVector();
...
controller_interface::CallbackReturn ElastoplasticController::on_init() {
  ...
  RCLCPP_DEBUG(get_node()->get_logger(),
               "Admittance HQP Controller correctly loaded");
...
void ElastoplasticController::configure_after_robot_description_callback(
  ...
  if (m_parameters.soft_limits.size() != m_arm_nax) {
    RCLCPP_ERROR_STREAM(get_node()->get_logger(),
                        "soft_limits parameters size != " << m_arm_nax);
    m_robot_description_configuration = RDStatus::ERROR;
    return;
  }
  ...
  for (size_t ax = 0; ax < m_arm_nax; ++ax) {
    ...
    if (elastoplastic::utils::almost_zero(m_limits.pos_upper(ax)) &&
      ...
      RCLCPP_WARN(get_node()->get_logger(),
                  "joint %ld upper and lower limits are both equal to 0, set "
                  "+/- infinity",
                  ax);
    }
  ...
  RCLCPP_INFO(this->get_node()->get_logger(),
              "Nq: %d, Nv: %d, Manipulator NAx: %ld", m_model.nq, m_model.nv,
              m_arm_nax);
  ...
  for (size_t idx = 0; idx < 6; idx++) {
    m_impedance.invM.diagonal()(idx) =
        1.0 / m_parameters.impedance.inertia[idx];
...
controller_interface::CallbackReturn ElastoplasticController::on_configure(
  ...
  // controller_manager
  // auto get_update_rate() =
  // this->get_node()->get_parameter("get_update_rate()").as_int();
  ...
  if (m_parameters.wrench.lpf.enable) {
    for (int idx = 0; idx < m_low_pass_filters.size(); idx++) {
      m_low_pass_filters.at(idx).activateFilter(
          m_parameters.wrench.deadband.at(0), 50.0, m_parameters.wrench.lpf.fc,
          m_dt, 0.0);
    }
  ...
  m_carteisan_trj_sub =
      get_node()->create_subscription<moveit_msgs::msg::CartesianTrajectory>(
          ...
          [this](const moveit_msgs::msg::CartesianTrajectory &msg) {
            ...
            m_interpolator =
                elastoplastic::utils::interpolation::Interpolator::from_msg(
                    msg);
            RCLCPP_INFO_STREAM(get_node()->get_logger(),
...
controller_interface::CallbackReturn ElastoplasticController::on_activate(
    ...
    if (m_parameters.wrench.lpf.enable) {
      for (int idx = 0; idx < m_low_pass_filters.size(); idx++) {
        m_low_pass_filters.at(idx).activateFilter(
            ...
            m_parameters.wrench.lpf.fc, m_dt, 0.0);
      }
  ...
  if (m_mobile_base->enabled) {
    ...
    m_q(2) =
        std::cos(elastoplastic::utils::vector_from_affine(T_world_base)(5));
    m_q(3) =
        std::sin(elastoplastic::utils::vector_from_affine(T_world_base)(5));
    m_qp.head<4>().setZero();
  }
  ...
  m_offset_future = std::async(std::launch::async, [this](void) -> bool {
    ...
    for (int idx = 0; idx < offset_force_window; ++idx) {
      ...
      // accumulate
      std::transform(wr.begin(), wr.end(), result_in_sensor.begin(),
                     result_in_sensor.begin(), std::plus<double>{});
    ...
    const pinocchio::SE3 tool_M_sensor =
        world_M_tool.inverse() * world_M_sensor;
    pinocchio::Force offset_wrench_tool_in_tool =
        tool_M_sensor.act(offset_wrench_sensor_in_sensor);
    const pinocchio::SE3 world_R_tool(world_M_tool.rotation(),
                                      Eigen::Vector3d::Zero());
    m_offset_wrench_tool_in_world =
        world_R_tool.act(offset_wrench_tool_in_tool).toVector();
  ...
  m_elastoplastic_model = std::make_unique<ElastoplasticModel>(
      m_impedance.K, m_parameters.impedance.z_max,
      m_parameters.impedance.z_kmax, m_parameters.impedance.z_start,
      static_cast<size_t>(m_parameters.impedance.reset.time *
                          get_update_rate()),
  ...
  m_T_tool_sensor =
      (world_M_tool.inverse() * world_M_sensor).toHomogeneousMatrix();
  ...
  m_computed_target_T_world_tool = world_M_tool.toHomogeneousMatrix();
  ...
  m_ref_T_world_base =
      Eigen::Affine3d(m_model_data.oMf[m_base_id].toHomogeneousMatrix());
  m_init_T_world_tool =
      Eigen::Affine3d(m_model_data.oMf[m_tool_id].toHomogeneousMatrix());
...
controller_interface::CallbackReturn ElastoplasticController::on_deactivate(
  ...
  pinocchio::framesForwardKinematics(m_model, m_model_data, m_q);
  m_reference_target_T_world_tool =
      m_model_data.oMf[m_tool_id].toHomogeneousMatrix();
  m_reference_target_acc_tool_world_in_world.setZero();
  m_reference_target_twist_tool_world_in_world.setZero();
...
ElastoplasticController::update_reference_from_subscribers() {
  ...
  std::copy(m_q.tail(m_arm_nax).begin(), m_q.tail(m_arm_nax).end(),
            reference_interfaces_.begin()); // position
  std::fill(std::next(reference_interfaces_.begin(), m_arm_nax),
...
ElastoplasticController::update_and_write_commands(
  ...
  std::transform(m_joint_state_interfaces.at(0).begin(),
                 m_joint_state_interfaces.at(0).end(),
                 m_q_in.tail(m_arm_nax).begin(),
                 [](const hardware_interface::LoanedStateInterface &lsi) {
                   return GET_VALUE_FROM_INTERFACE(lsi);
                 });
  std::transform(m_joint_state_interfaces.at(1).begin(),
                 m_joint_state_interfaces.at(1).end(),
                 m_qp_in.tail(m_arm_nax).begin(),
                 [](const hardware_interface::LoanedStateInterface &lsi) {
                   return GET_VALUE_FROM_INTERFACE(lsi);
                 });
  std::transform(m_joint_state_interfaces.at(2).begin(),
                 m_joint_state_interfaces.at(2).end(),
                 m_tau_in.tail(m_arm_nax).begin(),
                 [](const hardware_interface::LoanedStateInterface &lsi) {
                   return GET_VALUE_FROM_INTERFACE(lsi);
                 });
  ...
  pinocchio::forwardKinematics(m_model, m_model_data, m_q, m_qp,
                               Eigen::VectorXd::Zero(m_model.nv));
  pinocchio::computeJointJacobians(m_model, m_model_data, m_q);
  ...
  if (m_interpolator.is_plan_started()) {
    [[maybe_unused]] auto status =
        m_interpolator.interpolate(get_node()->get_clock()->now(),
                                   m_reference_target_acc_tool_world_in_world,
                                   m_reference_target_twist_tool_world_in_world,
                                   m_reference_target_T_world_tool);
  }

  if (m_mobile_base->enabled) {
    m_full_velocity_references.head<M_SE2>() = utils::base_velocity_from_twist(
        m_computed_target_twist_tool_world_in_world);

    Eigen::Vector3d base_ref =
        utils::base_velocity_from_twist(utils::vector_from_affine(
            m_computed_target_T_world_tool * m_init_T_world_tool.inverse() *
            m_ref_T_world_base));
    m_full_position_references.head<2>() = base_ref.head<2>();
    m_full_position_references(2) = std::cos(base_ref(2));
    m_full_position_references(3) = std::sin(base_ref(2));
  }
  // TODO: end plan
  ...
  if (wrench_tool_in_world.hasNaN()) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "Wrench has NaNs... Skipping update");
  ...
  Eigen::Vector6d twist_tool_world_in_world =
      pin::getFrameVelocity(m_model, m_model_data, m_tool_id,
  ...
  bool reset = m_elastoplastic_model->reset(
      wrench_tool_in_world.cwiseProduct(m_impedance.enabled_axis),
  ...
  if (m_offset_future.wait_for(0s) != std::future_status::ready) {
    ...
    // tau_cmd = pinocchio::computeGeneralizedGravity(m_model, m_model_data,
    // m_q_in); tau_cmd.head<3>().setZero();
    tau_cmd.setZero();
    bool result{true};
  ...
  if (!solution_qp.has_value()) {
    ...
    m_computed_target_acc_tool_world_in_world = solution_qp.value().tail<6>();
  ...
  Eigen::Affine3d comp_target_pose = utils::integratePoseLocalWorldAligned(
      pin::SE3(m_computed_target_T_world_tool.matrix()),
      pin::Motion(m_computed_target_twist_tool_world_in_world), m_dt);
  ...
  //     RCLCPP_WARN(get_node()->get_logger(),
  //                 "Saturation at %f of POSITION (cmd: %f) on manipulator
  //                 joint " "with index %ld", q, m_q(idx + (m_model.nq -
  //                 m_arm_nax)), idx);
  //   }
  //   if (!elastoplastic::utils::almost_equal(
  //           dq, m_qp(idx + (m_model.nv - m_arm_nax)))) {
  //     RCLCPP_WARN(get_node()->get_logger(),
  //                 "Saturation of VELOCITY on manipulator joint with index
  //                 %ld", idx);
  //   }
  // }
  ...
  if (m_used_command_interfaces.at(0)) {
    ...
    cmd = tau_cmd + m_parameters.gains.k1 * (m_q - m_q_in) +
          m_parameters.gains.k2 * (m_qp - m_qp_in) +
          m_parameters.gains.k3 * m_qpp;
    if (m_parameters.remove_gravity_from_torque_cmd) {
      cmd -=
          pinocchio::computeGeneralizedGravity(m_model, m_model_data, m_q_in);
    }
  }
  ...
  if (m_mobile_base->enabled) {
    [[maybe_unused]] bool is_mobile_base_write_ok =
        write_cmd_vel(m_qp.head<3>());
  }
  ...
  msg.cart_actual_cmd_pose = tf2::toMsg(Eigen::Affine3d(
      pinocchio::updateFramePlacement(m_model, computed_data, m_tool_id)
          .toHomogeneousMatrix()));
  msg.cart_actual_cmd_twist = tf2::toMsg(pinocchio::getFrameVelocity(
      m_model, computed_data, m_tool_id,
      pinocchio::ReferenceFrame::LOCAL_WORLD_ALIGNED));
  msg.cart_actual_cmd_acc = tf2::toMsg(pinocchio::getFrameAcceleration(
      m_model, computed_data, m_tool_id,
      pinocchio::ReferenceFrame::LOCAL_WORLD_ALIGNED));

  pinocchio::forwardKinematics(m_model, input_data, m_q_in, m_qp_in);
  msg.cart_actual_pose = tf2::toMsg(Eigen::Affine3d(
      pinocchio::updateFramePlacement(m_model, input_data, m_tool_id)
          .toHomogeneousMatrix()));
  msg.cart_actual_twist = tf2::toMsg(pinocchio::getFrameVelocity(
      m_model, input_data, m_tool_id,
      pinocchio::ReferenceFrame::LOCAL_WORLD_ALIGNED));
  ...
  std::ranges::copy(m_full_position_references,
                    std::back_inserter(msg.joint_reference.position));
  std::ranges::copy(m_full_velocity_references,
                    std::back_inserter(msg.joint_reference.velocity));

  msg.admittance_state.selected_axes.data.reserve(6);
  std::copy(m_impedance.enabled_axis.begin(), m_impedance.enabled_axis.end(),
  ...
  Eigen::Matrix6d K =
      m_elastoplastic_model->compute_variable_matrices(T_world_tool);
