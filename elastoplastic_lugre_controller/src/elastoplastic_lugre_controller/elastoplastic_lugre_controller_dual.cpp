...
get_model_data(const elastoplastic_controller_dual::Params &params,
...
void ElastoplasticControllerDual::update_grasp_matrices(
    const Eigen::Matrix3d &R_world_shared) {
  m_grasp_matrix_wrench.block<3, 3>(Eigen::fix<3>, Eigen::fix<0>) =
      rdyn::skew(R_world_shared * m_p_left_shared_in_shared);
  m_grasp_matrix_wrench.block<3, 3>(Eigen::fix<3>, Eigen::fix<6>) =
      rdyn::skew(R_world_shared * m_p_right_shared_in_shared);
  m_grasp_matrix_twist.block<3, 3>(Eigen::fix<0>, Eigen::fix<3>) =
      0.5 * rdyn::skew(R_world_shared * -m_p_left_shared_in_shared);
  m_grasp_matrix_twist.block<3, 3>(Eigen::fix<0>, Eigen::fix<9>) =
      0.5 * rdyn::skew(R_world_shared * -m_p_right_shared_in_shared);
}

controller_interface::CallbackReturn ElastoplasticControllerDual::on_init() {
  m_param_listener =
      std::make_shared<elastoplastic_controller_dual::ParamListener>(
          this->get_node());
...
void ElastoplasticControllerDual::configure_after_robot_description_callback(
  ...
  // rdyn::ChainPtr chain_base_fork = rdyn::createChain(*urdf_model,
  // m_parameters.frames.base, m_parameters.frames.fork, gravity);

  // rdyn::ChainPtr chain_fork_tool =
  //   rdyn::createChain(*urdf_model, m_parameters.frames.fork,
  //   m_parameters.frames.tools[Side::LEFT], gravity);
  // m_chain_base_tools[Side::LEFT] = rdyn::joinChains(chain_base_fork,
  // chain_fork_tool); chain_fork_tool = rdyn::createChain(*urdf_model,
  // m_parameters.frames.fork, m_parameters.frames.tools[Side::RIGHT], gravity);
  // m_chain_base_tools[Side::RIGHT] = rdyn::joinChains(chain_base_fork,
  // chain_fork_tool);

  m_chain_base_tools[Side::LEFT] =
      rdyn::createChain(*urdf_model, m_parameters.frames.base,
                        m_parameters.frames.tools[Side::LEFT], gravity);
  m_chain_base_tools[Side::RIGHT] =
      rdyn::createChain(*urdf_model, m_parameters.frames.base,
                        m_parameters.frames.tools[Side::RIGHT], gravity);

  // rdyn::ChainPtr chain_fork_sensor =
  //   rdyn::createChain(*urdf_model, m_parameters.frames.fork,
  //   m_parameters.frames.sensors[Side::LEFT], gravity);
  // m_chain_base_sensors[Side::LEFT] = rdyn::joinChains(chain_base_fork,
  // chain_fork_sensor); chain_fork_sensor = rdyn::createChain(*urdf_model,
  // m_parameters.frames.fork, m_parameters.frames.sensors[Side::RIGHT],
  // gravity); m_chain_base_sensors[Side::RIGHT] =
  // rdyn::joinChains(chain_base_fork, chain_fork_sensor);
  ...
  m_chain_base_sensors[Side::RIGHT] =
      rdyn::createChain(*urdf_model, m_parameters.frames.base,
                        m_parameters.frames.sensors[Side::RIGHT], gravity);

  if (std::ranges::find(m_chain_base_tools, nullptr) !=
      m_chain_base_tools.end()) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "Cannot create rdyn chain from base to tool");
    m_robot_description_configuration = RDStatus::ERROR;
    return;
  }
  if (std::ranges::find(m_chain_base_sensors, nullptr) !=
      m_chain_base_sensors.end()) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "Cannot create rdyn chain from base to sensor");
    m_robot_description_configuration = RDStatus::ERROR;
    return;
  }
  ...
  if (m_mobile_base->enabled) {
    ...
    rdyn::ChainPtr chain_world_base = rdyn::createChain(
        *mobile_base_model, "x_base", "mount_link", {0, 0, -9.806});
    for (const auto &side : Side::arms()) {
      m_chain_world_tools[side] =
          rdyn::joinChains(chain_world_base, m_chain_base_tools[side]);
    }
  } else {
    m_chain_world_tools[Side::LEFT] =
        rdyn::createChain(*urdf_model, m_parameters.frames.map,
                          m_parameters.frames.tools[Side::LEFT], gravity);
  ...
  if (std::ranges::find(m_chain_world_tools, nullptr) !=
      m_chain_world_tools.end()) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "Cannot create rdyn chain from base to tool");
    m_robot_description_configuration = RDStatus::ERROR;
    return;
  ...
  m_joint_names.resize(m_parameters.joints.left.size() +
                       m_parameters.joints.right.size() +
                       m_parameters.joints.common.size());
  auto it =
      std::ranges::copy(m_parameters.joints.common, m_joint_names.begin());
  ...
  for (size_t ax = 0; ax < m_nax; ++ax) {
    m_limits.pos_upper(ax) =
        urdf_model->getJoint(m_joint_names.at(ax))->limits->upper;
    m_limits.pos_lower(ax) =
        urdf_model->getJoint(m_joint_names.at(ax))->limits->lower;
    ...
    m_limits.vel(ax) =
        urdf_model->getJoint(m_joint_names.at(ax))->limits->velocity;
    m_limits.acc(ax) =
  ...
  std::string what;
  std::vector<std::string> v(m_mobile_base->nax() +
                             m_parameters.joints.left.size() +
                             m_parameters.joints.common.size());
  std::vector<std::string> vb(v.size() - m_mobile_base->nax());

  auto it_jnt_names =
      std::ranges::copy(m_mobile_base->base_joint_names, v.begin());
  it_jnt_names =
      std::ranges::copy(m_parameters.joints.common, it_jnt_names.out);
  ...
  v.resize(m_mobile_base->nax() + m_parameters.joints.right.size() +
           m_parameters.joints.common.size());
...
controller_interface::CallbackReturn ElastoplasticControllerDual::on_configure(
  ...
  m_nax_s[Side::LEFT] =
      m_parameters.joints.left.size() + m_split_nax[Side::COMMON];
  m_nax_s[Side::RIGHT] =
      m_parameters.joints.right.size() + m_split_nax[Side::COMMON];
  ...
  m_nax = m_split_nax[Side::LEFT] + m_split_nax[Side::RIGHT] +
          m_split_nax[Side::COMMON];
  ...
  // Select appropriate elements for computations on individual chains:
  // m_q(m_selector[Side::LEFT]) --> q[base, commmon, left]
  m_sel[Side::LEFT].resize(m_split_nax[Side::COMMON] + m_split_nax[Side::LEFT] +
                           m_split_nax[Side::BASE]);
  std::iota(m_sel[Side::LEFT].begin(),
            std::next(m_sel[Side::LEFT].begin(), m_split_nax[Side::BASE]), 0);
  std::iota(std::next(m_sel[Side::LEFT].begin(), m_idx_st[Side::COMMON]),
            std::next(m_sel[Side::LEFT].begin(),
                      m_idx_st[Side::COMMON] + m_split_nax[Side::COMMON]),
            m_idx_st[Side::COMMON]);
  std::iota(std::next(m_sel[Side::LEFT].begin(),
                      m_split_nax[Side::BASE] + m_split_nax[Side::COMMON]),
            std::next(m_sel[Side::LEFT].begin(), m_split_nax[Side::BASE] +
                                                     m_split_nax[Side::COMMON] +
                                                     m_split_nax[Side::LEFT]),
            m_idx_st[Side::LEFT]);

  m_sel[Side::RIGHT].resize(m_split_nax[Side::COMMON] +
                            m_split_nax[Side::RIGHT] + m_split_nax[Side::BASE]);
  std::iota(m_sel[Side::RIGHT].begin(),
            std::next(m_sel[Side::RIGHT].begin(), m_split_nax[Side::BASE]), 0);
  std::iota(std::next(m_sel[Side::RIGHT].begin(), m_split_nax[Side::BASE]),
            std::next(m_sel[Side::RIGHT].begin(),
                      m_split_nax[Side::BASE] + m_split_nax[Side::COMMON]),
            m_idx_st[Side::COMMON]);
  std::iota(std::next(m_sel[Side::RIGHT].begin(),
                      m_split_nax[Side::BASE] + m_split_nax[Side::COMMON]),
            std::next(m_sel[Side::RIGHT].begin(),
                      m_split_nax[Side::BASE] + m_split_nax[Side::COMMON] +
                          m_split_nax[Side::RIGHT]),
            m_idx_st[Side::RIGHT]);
  ...
  m_sub_mobile_base_odometry =
      this->get_node()->create_subscription<nav_msgs::msg::Odometry>(
          ...
          std::bind(&ElastoplasticControllerDual::get_odometry_callback, this,
                    _1));
  ...
  m_ft_sensors[Side::LEFT] =
      std::make_unique<semantic_components::ForceTorqueSensor>(
          m_parameters.ft_sensor_names.at(0));
  m_ft_sensors[Side::RIGHT] =
      std::make_unique<semantic_components::ForceTorqueSensor>(
          m_parameters.ft_sensor_names.at(1));
  ...
  if (get_node()->has_parameter("robot_description")) {
    ...
    m_sub_robot_description =
        get_node()->create_subscription<std_msgs::msg::String>(
            ...
            std::bind(&ElastoplasticControllerDual::
  ...
  m_cartesian_pos_limits = Eigen::Map<Eigen::Vector6d>(
      m_parameters.cartesian_limits.position.data(), 6);
  ...
  // std::make_unique<std::thread>(&ElastoplasticController::update_base_pose_from_tf,
  // this);
  if (m_mobile_base->enabled) {
...
void ElastoplasticControllerDual::update_base_pose_from_tf() {
  ...
  //                 this->get_node()
  //                     ->get_name())); // lifecycle nodes cannot create
  //                     sub_nodes
  // m_node_support->set_parameter(get_node()->get_parameter("use_sim_time"));
  // m_tf_listener = std::make_shared<tf2_ros::TransformListener>(
  //     *m_tf_buffer, m_node_support, false);
  // m_support_node_exec =
...
ElastoplasticControllerDual::state_interface_configuration() const {
  controller_interface::InterfaceConfiguration state_interface_configuration;
  state_interface_configuration.type =
      controller_interface::interface_configuration_type::INDIVIDUAL;

  state_interface_configuration.names.reserve(
      (m_parameters.joints.left.size() + m_parameters.joints.right.size()) *
          m_allowed_interface_types.size() +
      12);
  ...
  for (const auto &side : Side::arms()) {
    std::vector<std::string> ft_interfaces =
        m_ft_sensors[side]->get_state_interface_names();
...
ElastoplasticControllerDual::command_interface_configuration() const {
  ...
  if (std::ranges::find(m_command_interfaces_names,
    ...
    for (const auto &jnt : m_joint_names) {
      command_interface_configuration.names.emplace_back(
          fmt::format("{}/{}", jnt, m_allowed_interface_types[1]));
    }
  }
...
controller_interface::CallbackReturn ElastoplasticControllerDual::on_activate(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  ...
  for (const auto &interface : m_allowed_interface_types) {
    ...
    if (not controller_interface::get_ordered_interfaces(
            state_interfaces_, m_joint_names, interface,
            m_joint_state_interfaces.at(idx))) {
      RCLCPP_ERROR(
          get_node()->get_logger(),
          "Missing joints state interfaces: %ld names vs %ld interfaces", m_nax,
  ...
  for (const auto &interface : m_allowed_interface_types) {
    ...
    auto idx = std::distance(m_allowed_interface_types.begin(), it);
    if (not controller_interface::get_ordered_interfaces(
            command_interfaces_, m_joint_names, interface,
  ...
  if (!m_ft_sensors[Side::LEFT]->assign_loaned_state_interfaces(
          state_interfaces_) ||
      !m_ft_sensors[Side::RIGHT]->assign_loaned_state_interfaces(
          state_interfaces_)) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "Cannot assing state interface to an ft_sensor");
    return controller_interface::CallbackReturn::ERROR;
  }
  ...
  try {
    ...
    RCLCPP_ERROR_STREAM(get_node()->get_logger(),
                        "Cannot get transformation from "
                            << m_parameters.frames.base << " to "
                            << m_parameters.frames.map
  ...
  Eigen::Affine3d T_world_left =
      m_chain_world_tools[Side::LEFT]->getTransformation(
          m_q(m_sel[Side::LEFT]));
  Eigen::Affine3d T_world_right =
      m_chain_world_tools[Side::RIGHT]->getTransformation(
          m_q(m_sel[Side::RIGHT]));
  ...
  // Eigen::Vector3d p_right_shared_in_world =
  // -utils::get_frame_distance(T_world_shared, T_world_right).head<3>();

  m_p_left_shared_in_shared =
      T_world_shared.linear().transpose() *
      (T_world_left.translation() - T_world_shared.translation());
  m_p_right_shared_in_shared =
      T_world_shared.linear().transpose() *
      (T_world_right.translation() - T_world_shared.translation());

  m_grasp_matrix_wrench << Eigen::Matrix3d::Identity(), Eigen::Matrix3d::Zero(),
      Eigen::Matrix3d::Identity(), Eigen::Matrix3d::Zero(),
      Eigen::Matrix3d::Identity(), rdyn::skew(m_p_left_shared_in_shared),
      Eigen::Matrix3d::Identity(), rdyn::skew(m_p_left_shared_in_shared);

  m_grasp_matrix_twist << Eigen::Matrix3d::Identity(),
      -rdyn::skew(m_p_left_shared_in_shared), Eigen::Matrix3d::Identity(),
      -rdyn::skew(m_p_left_shared_in_shared), Eigen::Matrix3d::Zero(),
      Eigen::Matrix3d::Identity(), Eigen::Matrix3d::Zero(),
      Eigen::Matrix3d::Identity();
  m_grasp_matrix_twist *= 0.5;

  // m_grasp_matrix_wrench << Eigen::Matrix6d::Identity(),
  // Eigen::Matrix6d::Identity(); m_grasp_matrix_twist << 0.5 *
  // Eigen::Matrix6d::Identity(), 0.5 * Eigen::Matrix6d::Identity();

  m_computed_target_T_world_shared =
      get_shared_frame(m_chain_world_tools[Side::LEFT]->getTransformation(
                           m_q(m_sel[Side::LEFT])),
                       m_chain_world_tools[Side::RIGHT]->getTransformation(
                           m_q(m_sel[Side::RIGHT])));
  ...
  m_offset_future = std::async(std::launch::async, [this](void) -> bool {
    ...
    Eigen::Vector12d exp_filter_prec_state = Eigen::Vector12d::Zero();
    ...
    for (int idx = 0; idx < offset_force_window; ++idx) {
      ...
      // accumulate
      std::transform(
          wr.begin(), wr.end(), m_offset_wrench_sensor_in_sensor.begin(),
          m_offset_wrench_sensor_in_sensor.begin(), std::plus<double>{});
    ...
    for (const auto &side : Side::arms()) {
      Eigen::Vector6d offset_wrench_tool_in_tool =
          rdyn::spatialDualTranformation(
              ...
              m_chain_base_tools[side]
                      ->getTransformation(m_q(m_sel[side]).tail(m_nax_s[side]))
                      .inverse() *
                  m_chain_base_sensors[side]->getTransformation(
                      m_q(m_sel[side]).tail(m_nax_s[side])));
      m_offset_wrench_tool_in_world.segment<6>(side * 6) =
          rdyn::spatialRotation(offset_wrench_tool_in_tool,
                                m_chain_world_tools[side]
                                    ->getTransformation(m_q(m_sel[side]))
                                    .linear());
    }
...
controller_interface::CallbackReturn ElastoplasticControllerDual::on_cleanup(
    const rclcpp_lifecycle::State & /*previous_state*/) {
...
controller_interface::CallbackReturn ElastoplasticControllerDual::on_error(
    const rclcpp_lifecycle::State &previous_state) {
  return ElastoplasticControllerDual::on_deactivate(previous_state);
}

std::vector<hardware_interface::CommandInterface>
ElastoplasticControllerDual::on_export_reference_interfaces() {
  std::vector<hardware_interface::CommandInterface> reference_interfaces;

  m_joint_reference_interfaces_size =
      m_joint_names.size() *
      m_allowed_interface_types.size(); // There must be both position and
                                        // velocity reference interfaces!
...
ElastoplasticControllerDual::update_reference_from_subscribers(
...
void ElastoplasticControllerDual::get_odometry_callback(
    const nav_msgs::msg::Odometry &msg) {
  m_rt_buffer_base_odom.writeFromNonRT(msg);
}

controller_interface::return_type
ElastoplasticControllerDual::update_and_write_commands(
    const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/) {
  ...
  T_world_tool[Side::LEFT] = m_chain_world_tools[Side::LEFT]->getTransformation(
      m_q(m_sel[Side::LEFT]));
  T_world_tool[Side::RIGHT] =
      m_chain_world_tools[Side::RIGHT]->getTransformation(
          m_q(m_sel[Side::RIGHT]));
  twist_tool_world_in_world.head<6>() =
      m_chain_world_tools[Side::LEFT]->getTwistTool(m_q(m_sel[Side::LEFT]),
                                                    m_qp(m_sel[Side::LEFT]));
  twist_tool_world_in_world.tail<6>() =
      m_chain_world_tools[Side::RIGHT]->getTwistTool(m_q(m_sel[Side::RIGHT]),
                                                     m_qp(m_sel[Side::RIGHT]));
  ...
  if (m_interpolator.is_plan_started()) {
    auto status =
        m_interpolator.interpolate(get_node()->get_clock()->now(),
                                   reference_target_acc_shared_world_in_world,
                                   reference_target_twist_shared_world_in_world,
    ...
    if (status != utils::interpolation::Interpolator::InterpolationResult::OK) {
      ...
      reference_target_T_world_shared =
          get_shared_frame(m_chain_world_tools[Side::LEFT]->getTransformation(
                               m_initial_q(m_sel[Side::LEFT])),
                           m_chain_world_tools[Side::RIGHT]->getTransformation(
                               m_initial_q(m_sel[Side::RIGHT])));
    }
  } else {
    reference_target_acc_shared_world_in_world.setZero();
    reference_target_twist_shared_world_in_world.setZero();
  ...
  if (m_mobile_base->enabled) {
    ...
    Eigen::Affine3d T_world_base_ref =
        m_chain_world_tools[Side::LEFT]->getTransformationLink(
            m_initial_q(m_sel[Side::LEFT]), m_parameters.frames.base);
    ...
    full_position_references.head<M_SE2>() =
        utils::base_velocity_from_twist(utils::vector_from_affine(
            m_computed_target_T_world_shared * T_world_shared_ref.inverse() *
            T_world_base_ref));
  }
  ...
  // Exponential filter
  std::transform(wrench_sensor_in_sensor.begin(), wrench_sensor_in_sensor.end(),
  ...
  for (const auto &side : Side::arms()) {
    Eigen::Affine3d T_base_tool = m_chain_base_tools[side]->getTransformation(
        m_q(m_sel[side]).tail(m_nax_s[side]));
    ...
    wrench_tool_in_tool.segment<6>(side * 6) = rdyn::spatialDualTranformation(
        wrench_sensor_in_sensor.segment<6>(side * 6), T_tool_sensor);

    wrench_tool_in_world.segment<6>(side * 6) =
        rdyn::spatialRotation(wrench_tool_in_tool.segment<6>(side * 6),
                              T_world_tool[side].linear()) -
        m_offset_wrench_tool_in_world.segment<6>(side * 6);

    // Deadband
    std::transform(wrench_tool_in_world.begin(), wrench_tool_in_world.end(),
                   m_deadbands.begin(), wrench_tool_in_world.begin(),
                   [](const double w, const double deadband) {
                     return std::abs(w) > deadband
                                ? utils::sgn(w) * (std::abs(w) - deadband)
                                : 0.0;
                   });
  }
  m_wrench_in_sensor_prec = wrench_sensor_in_sensor;
  Eigen::Vector6d wrench_shared_in_world =
      m_grasp_matrix_wrench * wrench_tool_in_world;
  // Eigen::Vector6d wrench_shared_offset = m_grasp_matrix_wrench *
  // m_offset_wrench_tool_in_world;
  ...
  J_world_tool_in_world << Jtmp1.leftCols(m_split_nax[Side::BASE]),
      ...
      Jtmp1.middleCols(m_idx_st[Side::LEFT], m_split_nax[Side::LEFT]),
      Eigen::Matrix6Xd::Zero(6, m_split_nax[Side::RIGHT]), //
      Jtmp2.leftCols(m_split_nax[Side::BASE]),
      Jtmp2.middleCols(m_idx_st[Side::COMMON], m_split_nax[Side::COMMON]),
      Eigen::Matrix6Xd::Zero(6, m_split_nax[Side::LEFT]), //
      Jtmp2.rightCols(m_split_nax[Side::RIGHT]);

  Eigen::Vector6d twist_shared_world_in_world =
      m_grasp_matrix_twist * twist_tool_world_in_world;

  Eigen::Vector6d cart_vel_error_shared_target_in_world =
      (twist_shared_world_in_world -
       m_computed_target_twist_shared_world_in_world)
          .cwiseProduct(m_elastoplastic_model->get_enabled_axis());

  m_zp = m_elastoplastic_model->update_z(cart_vel_error_shared_target_in_world,
                                         m_dt);
  bool reset = m_elastoplastic_model->reset(
      wrench_shared_in_world.cwiseProduct(
          m_elastoplastic_model->get_enabled_axis()),
      cart_vel_error_shared_target_in_world);

  m_computed_target_T_world_shared =
      reset ? m_T_world_shared : m_computed_target_T_world_shared;
  m_computed_target_twist_shared_world_in_world =
      reset ? twist_shared_world_in_world
            : m_computed_target_twist_shared_world_in_world;
  ...
  ClikData clik_data{.position_references = full_position_references,
                     ...
                     .target_twist_shared_world_in_world =
                         reference_target_twist_shared_world_in_world,
  ...
  // m_computed_target_twist_shared_world_in_world +=
  // m_computed_target_acc_shared_world_in_world * m_dt;
  // m_computed_target_T_world_shared =
  // rdyn::spatialIntegration(m_computed_target_T_world_shared,
  // m_computed_target_twist_shared_world_in_world, m_dt);
  ...
  std::tie(cmp_t_wt, m_computed_target_twist_shared_world_in_world) =
      utils::rk4_double(
          [&](const auto &, const auto &, const auto &u) { return u; },
          cmp_t_wt, m_computed_target_twist_shared_world_in_world,
          m_computed_target_acc_shared_world_in_world, m_dt);
  m_computed_target_T_world_shared = utils::affine_from_vector(cmp_t_wt);

  Eigen::Vector6d dist;
  utils::get_frame_distance(m_T_world_shared, reference_target_T_world_shared,
                            dist);
  if (m_elastoplastic_model->to_restore() &&
      !m_elastoplastic_model->is_plastic() && dist.head<3>().norm() < 1e-2 &&
      dist.tail<3>().norm() < 1.0 &&
  ...
  msg.cart_actual_cmd_pose[ElastoplasticDualMsg::SIDE_LEFT] =
      tf2::toMsg(m_chain_world_tools[Side::LEFT]->getTransformation(
          m_q(m_sel[Side::LEFT])));
  msg.cart_actual_cmd_pose[ElastoplasticDualMsg::SIDE_RIGHT] =
      tf2::toMsg(m_chain_world_tools[Side::RIGHT]->getTransformation(
          m_q(m_sel[Side::RIGHT])));

  msg.cart_actual_cmd_twist[ElastoplasticDualMsg::SIDE_LEFT] =
      tf2::toMsg(m_chain_world_tools[Side::LEFT]->getTwistTool(
          m_q(m_sel[Side::LEFT]), m_qp(m_sel[Side::LEFT])));
  msg.cart_actual_cmd_twist[ElastoplasticDualMsg::SIDE_RIGHT] =
      tf2::toMsg(m_chain_world_tools[Side::RIGHT]->getTwistTool(
          m_q(m_sel[Side::RIGHT]), m_qp(m_sel[Side::RIGHT])));

  msg.cart_actual_cmd_acc[ElastoplasticDualMsg::SIDE_LEFT] =
      tf2::toMsg(m_chain_world_tools[Side::LEFT]->getDTwistTool(
          m_q(m_sel[Side::LEFT]), m_qp(m_sel[Side::LEFT]),
          m_qpp(m_sel[Side::LEFT])));
  msg.cart_actual_cmd_acc[ElastoplasticDualMsg::SIDE_RIGHT] =
      tf2::toMsg(m_chain_world_tools[Side::RIGHT]->getDTwistTool(
          m_q(m_sel[Side::RIGHT]), m_qp(m_sel[Side::RIGHT]),
  ...
  msg.cart_actual_tool_twist[ElastoplasticDualMsg::SIDE_LEFT] =
      tf2::toMsg(twist_tool_world_in_world.head<6>().eval());
  msg.cart_actual_tool_twist[ElastoplasticDualMsg::SIDE_RIGHT] =
      tf2::toMsg(twist_tool_world_in_world.tail<6>().eval());

  msg.cart_actual_tool_pose[ElastoplasticDualMsg::SIDE_LEFT] =
      tf2::toMsg(T_world_tool[Side::LEFT]);
  msg.cart_actual_tool_pose[ElastoplasticDualMsg::SIDE_RIGHT] =
      tf2::toMsg(T_world_tool[Side::RIGHT]);

  [[maybe_unused]] double unused_double;
  std::tie(msg.reset_buffer_state, unused_double) =
      m_elastoplastic_model->get_reset_buffer_status();

  msg.wrenches[ElastoplasticDualMsg::SIDE_LEFT] =
      utils::toWrenchMsg(wrench_tool_in_world.head<6>());
  msg.wrenches[ElastoplasticDualMsg::SIDE_RIGHT] =
      utils::toWrenchMsg(wrench_tool_in_world.tail<6>());

  msg.admittance_state.wrench_base =
      utils::toWrenchStampedMsg(wrench_shared_in_world);

  msg.admittance_state.admittance_position =
      tf2::eigenToTransform(m_computed_target_T_world_shared);
  ...
  msg.admittance_state.admittance_velocity.twist =
      tf2::toMsg(m_computed_target_twist_shared_world_in_world);
  ...
  msg.admittance_state.admittance_acceleration.twist =
      tf2::toMsg(m_computed_target_acc_shared_world_in_world);
  ...
  msg.admittance_state.joint_state.effort.reserve(m_joint_names.size() +
                                                  m_mobile_base->nax());
  std::copy(m_mobile_base->base_joint_names.begin(),
            m_mobile_base->base_joint_names.end(),
  ...
  std::copy(m_joint_names.begin(), m_joint_names.end(),
            std::back_inserter(msg.joint_reference.name));
  std::copy(full_position_references.begin(), full_position_references.end(),
            std::back_inserter(msg.joint_reference.position));
  std::copy(full_velocity_references.begin(), full_velocity_references.end(),
  ...
  auto [K, D] =
      m_elastoplastic_model->compute_variable_matrices(m_T_world_shared);
...
PLUGINLIB_EXPORT_CLASS(elastoplastic::ElastoplasticControllerDual,
