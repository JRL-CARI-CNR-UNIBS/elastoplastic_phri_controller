...
#include "elastoplastic_lugre_controller/interpolation/interpolation_data_types.hpp"
...
bool ElastoplasticController::write_cmd_vel(
    const Eigen::Ref<Eigen::Vector3d> &v) {
  bool b = true;
...
void ElastoplasticController::configure_after_robot_description_callback(
  ...
  joint_names_to_lock.erase(joint_names_to_lock.begin() +
                            1); // Rimuovi planar joint dalla lista
  ...
  m_chain_world_tool_data = pin::Data(m_chain_base_tool);

  // urdf::ModelInterfaceSharedPtr mobile_base_model =
  //     urdf::parseURDF(utils::MOBILE_BASE_URDF);
  // pin::urdf::buildModel(mobile_base_model, m_chain_world_base);
  // m_chain_world_base_data = pin::Data(m_chain_world_base);

  // pin::FrameIndex base_id = m_chain_world_base.getFrameId("mount_link");
  // pin::appendModel(m_chain_world_base, m_chain_base_tool, base_id,
  //                  pin::SE3::Identity(), m_chain_world_tool);
  // m_chain_world_tool_data = pin::Data(m_chain_world_tool);

  m_tool_id = m_chain_world_tool.getFrameId(m_parameters.frames.tool);
  if (m_chain_world_tool.nframes == m_tool_id) {
    RCLCPP_ERROR(get_node()->get_logger(),
  ...
  m_sensor_id = m_chain_world_tool.getFrameId(m_parameters.frames.sensor);
  if (m_chain_world_tool.nframes == m_sensor_id) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "Frame %s does not exist in the model",
  ...
  m_base_id = m_chain_world_tool.getFrameId(m_parameters.frames.base);
  if (m_chain_world_tool.nframes == m_base_id) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "Frame %s does not exist in the model",
  ...
  RCLCPP_INFO(get_node()->get_logger(), "Pinocchio model created");
  ...
  for (const auto &jnt : m_parameters.joints) {
    m_jnt_id.push_back(m_chain_world_tool.getFrameId(jnt));
  }
  m_jnt_base_id = {1};
  // for (const auto &jnt : {"move_x", "move_y", "rot_z"}) {
  //   m_jnt_base_id.push_back(m_chain_world_tool.getFrameId(jnt));
  // }
...
controller_interface::CallbackReturn ElastoplasticController::on_configure(
  ...
  m_q.resize(m_full_nax + 1);
  m_qp.resize(m_full_nax);
  m_qpp.resize(m_full_nax);

  m_q_prec.resize(m_full_nax + 1);
...
controller_interface::CallbackReturn ElastoplasticController::on_activate(
  ...
  std::transform(m_joint_state_interfaces.at(0).begin(),
                 m_joint_state_interfaces.at(0).end(), q.tail(m_nax).begin(),
  ...
  std::transform(m_joint_state_interfaces.at(1).begin(),
                 m_joint_state_interfaces.at(1).end(), dq.tail(m_nax).begin(),
                 [](const hardware_interface::LoanedStateInterface &lsi) {
                   return GET_VALUE_FROM_INTERFACE(lsi);
                 });
                 m_chain_world_tool.idx_qs
  m_q(m_jnt_id) = q;
  m_qp(m_jnt_id) = dq;
  m_qpp.setZero();
  ...
  Eigen::Vector3d qb;
  qb.head<2>() = m_T_world_base.translation().head<2>();
  qb(2) = std::cos(utils::vector_from_affine(m_T_world_base)(5));
  qb(3) = std::sin(utils::vector_from_affine(m_T_world_base)(5));
  m_q.head<4>() = qb;
  m_qp(m_jnt_base_id).setZero();
  ...
  // q(m_jnt_base_id).setZero();

  pin::computeForwardKinematicsDerivatives(
      m_chain_world_tool, m_chain_world_tool_data, m_q, m_qp, m_qpp);
  pin::updateFramePlacements(m_chain_world_tool, m_chain_world_tool_data);
  m_computed_target_T_world_tool =
      m_chain_world_base_data.oMf[m_tool_id].toHomogeneousMatrix();
  m_initial_T_world_tool = m_computed_target_T_world_tool;
  m_initial_T_world_base =
      m_chain_world_base_data.oMf[m_base_id].toHomogeneousMatrix();
  ...
  m_offset_future = std::async(std::launch::async, [this](void) -> bool {
    ...
    pin::Force offset_wrench_sensor_in_sensor__pin(
        offset_wrench_sensor_in_sensor);
    pin::SE3 T_sensor_tool__pin =
        m_chain_world_tool_data.oMf[m_tool_id].inverse() *
        m_chain_world_tool_data.oMf[m_sensor_id];
    pin::SE3 R_world_tool__pin = m_chain_world_tool_data.oMf[m_tool_id];
    R_world_tool__pin.rotation().setIdentity();
    pin::Force offset_wrench_tool_in_tool__pin =
        T_sensor_tool__pin.act(offset_wrench_sensor_in_sensor__pin);
    m_offset_wrench_tool_in_world =
        R_world_tool__pin.act(offset_wrench_tool_in_tool__pin);
  ...
  m_T_tool_sensor = (m_chain_world_tool_data.oMf[m_tool_id].inverse() *
                     m_chain_world_tool_data.oMf[m_sensor_id])
                        .toHomogeneousMatrix();
...
controller_interface::CallbackReturn ElastoplasticController::on_deactivate(
  ...
  Eigen::Vector3d qb;
  qb.head<2>() = m_T_world_base.translation().head<2>();
  qb(2) = utils::vector_from_affine(m_T_world_base)(5);
  m_q(m_jnt_base_id) = qb;
  m_qp(m_jnt_base_id).setZero();
  ...
  m_computed_target_T_world_tool =
      m_chain_world_tool_data.oMf[m_tool_id].toHomogeneousMatrix();
  m_computed_target_acc_tool_world_in_world.setZero();
  m_computed_target_twist_tool_world_in_world.setZero();
...
void ElastoplasticController::get_odometry_callback(
    const nav_msgs::msg::Odometry &msg) {
  m_rt_buffer_base_odom.writeFromNonRT(msg);
}

controller_interface::return_type
ElastoplasticController::update_and_write_commands(
    const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/) {
  ...
  q_in(m_jnt_base_id) = m_q(m_jnt_base_id);
  qp_in(m_jnt_base_id) = m_qp(m_jnt_base_id);
  for (int idx = 0; idx < m_joint_state_interfaces.at(0).size(); idx++) {
    q_in(m_jnt_id[idx]) =
        GET_VALUE_FROM_INTERFACE(m_joint_state_interfaces.at(0).at(idx).get());
    qp_in(m_jnt_id[idx]) =
        GET_VALUE_FROM_INTERFACE(m_joint_state_interfaces.at(1).at(idx).get());
  }
  ...
  T_world_tool = m_chain_world_tool_data.oMf[m_tool_id].toHomogeneousMatrix();
  Eigen::Vector6d twist_tool_world_in_world =
      pin::getFrameVelocity(m_chain_world_tool, m_chain_world_tool_data,
                            m_tool_id, pin::LOCAL_WORLD_ALIGNED);
  ...
  if (m_interpolator.is_plan_started()) {
    ...
    if (status != utils::interpolation::Interpolator::InterpolationResult::OK) {
      ...
      reference_target_T_world_tool = m_initial_T_world_tool;
    }
  } else {
    reference_target_acc_tool_world_in_world.setZero();
    reference_target_twist_tool_world_in_world.setZero();
  ...
  Eigen::Affine3d T_world_base_ref = m_initial_T_world_base;
  Eigen::Affine3d T_world_tool_ref = m_initial_T_world_tool;
  full_position_references.head<M_SE2>() = utils::base_velocity_from_twist(
      utils::vector_from_affine(m_computed_target_T_world_tool *
                                T_world_tool_ref.inverse() * T_world_base_ref));
  ...
  Eigen::Matrix6Xd J_world_tool_in_world =
      m_chain_world_tool_data.J; // Calcolato all'inizio della funzione
  ...
  pin::Force wrench_sensor_in_sensor__pin(wrench_sensor_in_sensor);
  pin::SE3 T_sensor_tool__pin =
      m_chain_world_tool_data.oMf[m_tool_id].inverse() *
      m_chain_world_tool_data.oMf[m_sensor_id];
  pin::SE3 R_world_tool__pin = m_chain_world_tool_data.oMf[m_tool_id];
  R_world_tool__pin.translation().setZero();
  pin::Force wrench_tool_in_tool__pin =
      T_sensor_tool__pin.act(wrench_sensor_in_sensor__pin);
  wrench_tool_in_world = R_world_tool__pin.act(wrench_tool_in_tool__pin);
  ...
  if (!solution_qp.has_value()) {
    ...
    m_qpp = qepp;
    m_computed_target_acc_tool_world_in_world =
        solution_qp.value().segment<M_SE3>(m_full_nax);
  }
  ...
  std::tie(cmp_t_wt, m_computed_target_twist_tool_world_in_world) =
      utils::rk4_double(
          ...
          cmp_t_wt, m_computed_target_twist_tool_world_in_world,
          m_computed_target_acc_tool_world_in_world, m_dt);
  ...
  Eigen::Vector6d dist;
  utils::get_frame_distance(T_world_tool, reference_target_T_world_tool, dist);
  Eigen::Vector6d qp_base_in_world = Eigen::Vector6d::Zero();
  qp_base_in_world = utils::twist_from_base_velocity(m_qp(m_jnt_base_id));

  pin::SE3 T_world_base__pin(m_T_world_base.matrix());
  T_world_base__pin.translation().setZero();

  Eigen::Vector6d qp_base_in_base =
      T_world_base__pin.inverse().act(qp_base_in_world);
      ...
      LOG_ERROR_THROTTLE_COUNT(
          ...
          "Saturation of Velocity on base linear direction "
              ...
              << utils::sgn(m_velocity_base_in_base(idx)) *
                     m_mobile_base->vel_limits(idx));
      m_velocity_base_in_base(idx) = utils::sgn(m_velocity_base_in_base(idx)) *
                                     m_mobile_base->vel_limits(idx);
    }
  }
  m_qp(m_jnt_base_id) = utils::base_velocity_from_twist(T_world_base__pin.act(
      utils::twist_from_base_velocity(m_velocity_base_in_base)));
  // END - Check Saturation Base

  // BEGIN - Saturation Manipulator
  for (size_t idx = 0; idx < m_nax; ++idx) {
    double q = m_q(m_jnt_id[idx]);
    double dq = m_qp(m_jnt_id[idx]);
    m_q(m_jnt_id[idx]) = std::max(
        m_limits.pos_lower(idx) - m_parameters.soft_limits.at(idx),
        std::min(m_limits.pos_upper(idx) + m_parameters.soft_limits.at(idx),
                 m_q(m_jnt_id[idx])));
    m_qp(m_jnt_id[idx]) = std::max(
        -m_limits.vel(idx), std::min(m_limits.vel(idx), m_qp(m_jnt_id[idx])));
    if (!utils::almost_equal(q, m_q(m_jnt_id[idx]))) {
      RCLCPP_WARN(get_node()->get_logger(),
                  "Saturation of POSITION on manipulator joint with index %ld",
                  idx);
    }
    if (!utils::almost_equal(dq, m_qp(m_jnt_id[idx]))) {
      RCLCPP_WARN(get_node()->get_logger(),
  ...
  if (m_mobile_base->enabled) {
    ...
    // integrate T_world_base
    m_T_world_base = [](const Eigen::Affine3d &T_b_a,
                        const Eigen::Ref<Eigen::Vector6d> &twist_of_a_in_b,
                        const double &dt) {
      Eigen::Affine3d T_b_ap = T_b_a;
      T_b_ap.translation() += twist_of_a_in_b.head(3) * dt;

      Eigen::Vector3d w_a_in_a =
          T_b_a.linear().transpose() * twist_of_a_in_b.tail(3);
      ...
      return T_b_ap;
    }(m_T_world_base, base_twist_in_base, m_dt);
  }
  ...
  msg.cart_actual_cmd_pose =
      tf2::toMsg((Eigen::Affine3d)m_chain_world_tool_data.oMf[m_tool_id]
                     .toHomogeneousMatrix());
  msg.cart_actual_cmd_twist = tf2::toMsg(
      pin::getFrameVelocity(m_chain_world_tool, m_chain_world_tool_data,
                            m_tool_id, pin::LOCAL_WORLD_ALIGNED));
  msg.cart_actual_cmd_acc = tf2::toMsg(
      pin::getFrameAcceleration(m_chain_world_tool, m_chain_world_tool_data,
                                m_tool_id, pin::LOCAL_WORLD_ALIGNED));

  pin::computeForwardKinematicsDerivatives(
      m_chain_world_tool, m_chain_world_tool_data, q_in, qp_in, m_qpp);
  pin::updateFramePlacements(m_chain_world_tool, m_chain_world_tool_data);
...
PLUGINLIB_EXPORT_CLASS(elastoplastic::ElastoplasticController,
