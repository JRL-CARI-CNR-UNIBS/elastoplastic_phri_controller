...
public:
  ...
  void push_task(Task &t, const double relative_task_weight = 1.0) {
    ...
    m_stack.emplace_back(std::make_tuple(
        G * relative_task_weight * std::pow(m_level_step, m_level),
        F * relative_task_weight * std::pow(m_level_step, m_level),
        t.describe()));
  ...
  void insert_task(Task &t, const int level,
    ...
    auto [G, F] = t.update_task();
    m_stack.emplace_back(std::make_tuple(
        G * relative_task_weight * std::pow(m_level_step, level),
        F * relative_task_weight * std::pow(m_level_step, level),
        t.describe()));
...
public:
  ...
  void compute_set() {
    ...
    if (eq_size > 0) {
      this->reset(eq_size);
      // m_CE(Eigen::seqN(0, m_eq.at(0).get().size()), Eigen::all) <<
      // m_eq.at(0).get().A(); m_ce.segment(0, m_eq.at(0).get().size()) <<
      // m_eq.at(0).get().b();
      size_t level = 0;
      for (size_t idx = 0; idx < m_eq.size(); ++idx) {
        // m_CE(Eigen::seqN(m_eq.at(idx - 1).get().size(),
        // m_eq.at(idx).get().size()), Eigen::all) << m_eq.at(idx).get().A();
        // m_ce.segment(m_eq.at(idx - 1).get().size(),
        // m_eq.at(idx).get().size()) << m_eq.at(idx).get().b();
...
public:
  ...
  size_t
  violations(const Eigen::VectorXd &x,
...
public:
  ...
  InequalityVectorRefConst
  which_violations(const Eigen::VectorXd &x,
                   const double toll = std::numeric_limits<double>::epsilon()) {
...
//     if (is_valid()) {
//       throw std::runtime_error("QP resulted in an error. Cannot get
//       solution");
//     }
