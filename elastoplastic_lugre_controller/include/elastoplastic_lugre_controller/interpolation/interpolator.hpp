...
class Interpolator {
public:
  enum class State { Empty, Available, Started, Terminated };

  enum class InterpolationResult { OK, InterpolatorNotStarted };

  Interpolator(const Trajectory &plan);
  Interpolator();

  void set_plan(const Trajectory &plan);
  Trajectory get_plan() { return m_plan; }
  static Interpolator
  from_msg(const moveit_msgs::msg::CartesianTrajectory &trj);
  static Interpolator from_msg(const moveit_msgs::msg::CartesianTrajectory &trj,
                               const Eigen::Affine3d &start_pose);

  void expect_plan_in_frame(const std::string &s) {
    m_expected_reference_frame = s;
  }
  ...
  Interpolator
  clone_with_transform(const geometry_msgs::msg::TransformStamped &tf);

  InterpolationResult interpolate(const rclcpp::Time &now, Eigen::Vector6d &acc,
