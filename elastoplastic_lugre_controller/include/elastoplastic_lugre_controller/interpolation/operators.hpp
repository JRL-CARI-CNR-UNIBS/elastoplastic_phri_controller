...
{
  Eigen::Vector3d v;
  v << mat(2,1), mat(0,2), mat(1,0);
  return v;
}

inline
Eigen::Matrix3d A_th(const Eigen::Vector3d& v)
{
  return Eigen::Matrix3d::Identity() + 0.5 * skew(v) + (1 - (0.5*v.norm())/tan(0.5*v.norm()))/std::pow(v.norm(),2) * skew(v) * skew(v);
}

inline
Eigen::Matrix3d A_th_inv(const Eigen::Vector3d &v)
{
  const double angle = v.norm();
  return Eigen::Matrix3d::Identity() - (1-cos(angle))/std::pow(angle,2) * skew(v) + (angle-sin(angle))/std::pow(angle,3)*skew(v)*skew(v);
}
...
Eigen::Matrix3d rot_exp(const Eigen::Vector3d& r)
{
  ...
  return Eigen::Matrix3d::Identity() + sin(norm)/norm * skew(r) + (1-cos(norm))/(norm*norm) * skew(r) * skew(r);
}

inline
Eigen::Matrix3d ang_vel_coeff_in_body_frame(const Eigen::Matrix3d& r)
{
  const double norm = unskew(r).norm();
