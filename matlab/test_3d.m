...
% dim 0 -> tempo
% dim 1 -> stati
% dim 2 -> dimensioni cartesiane
z =   zeros(length(T), 3, 3);
x =   zeros(length(T), 1, 3);
xp =  zeros(length(T), 1, 3);
xpp = zeros(length(T), 1, 3);

fr = zeros(length(T), 1, 3);
...
%% Evolution
z = zeros(length(T), z_size, 3);
u = zeros(length(T), 1, 3);
zp_ = zeros(length(T), z_size, 3);

reset_window = [];

for idx = 1:length(T)-1
    u(idx, :, :) = [0;.5;1] * amp*input_force(T(idx)) + noise_amp * randn();
    [zp, fr(idx, :, :)] = elastoplastic_model(z(idx, :, :), xp(idx, :, :) - input_trajectory(idx, 2));
    z(idx+1, :, :) = squeeze(z(idx, :, :)) + zp * dt;
...
subplot(3,1,1)
hold on
plot(T, squeeze(u),"LineWidth",1.5);
plot(T, squeeze(fr),"LineWidth",1.5);
legend("Input force X", "Input force Y", "Input force Z", "Friction force X", "Friction force Y", "Friction force Z")
hold off
grid on
subplot(3,1,2)
hold on
plot(T, squeeze(xp),"--","LineWidth",1.5)
plot(T, squeeze(xpp),"LineWidth",1.5)
hold off
legend("xp(x)", "xp(y)", "xp(z)", "xpp(x)", "xpp(y)", "xpp(z)")
grid on
subplot(3,1,3)
plot(T, squeeze(x),"-.","LineWidth",1.5)
legend("x(x)", "x(y)", "x(z)")
grid on

figure
plot(T, squeeze(x) - ([1;1;1] * input_trajectory(:,1)')',"-","LineWidth",1.5)
legend("\Delta x", "\Delta y", "\Delta z")
grid on

figure
subplot(3,2,1)
plot(T, squeeze(zp_(:,:,1)), "LineWidth",1.5);
legend("zp", "wp", "rp")
title("X")
grid on
subplot(3,2,3)
plot(T, squeeze(zp_(:,:,2)), "LineWidth",1.5);
legend("zp", "wp", "rp")
title("Y")
grid on
subplot(3,2,5)
plot(T, squeeze(zp_(:,:,3)), "LineWidth",1.5);
legend("zp", "wp", "rp")
title("Z")
grid on
subplot(3,2,2)
plot(T, squeeze(z(:,:,1)), "LineWidth",1.5);
legend("z", "w", "r")
title("X")
grid on
subplot(3,2,4)
plot(T, squeeze(z(:,:,2)), "LineWidth",1.5);
legend("z", "w", "r")
title("Y")
grid on
subplot(3,2,6)
plot(T, squeeze(z(:,:,3)), "LineWidth",1.5);
legend("z", "w", "r")
title("Z")
grid on
...
function [zp_ext, fr] = elastoplastic(z_ext,v,s0,s1,s2,fss,z_ba,z_ss,kw)
    ...
    zp = v - A * norm(v) * s0/fss * z;
    rp = d_alpha(max([z r]), z_ba, z_ss)*z_ss; % kr? Mi scoccia aggiungere un altro parametro
    wp = A * kw * (z - w);
    fr = s0*(z-w) + s1*(zp) + s2*v;
    zp_ext = [zp'; wp'; ones(1,3)*rp];
end

function [out, reset_window] = reset_condition(dt, z_ext, reset_window, v, fh, reset_window_size, reset_ths)
    z = z_ext(1, :, :);
    reset_window = [reset_window sum(fh .* v)];
